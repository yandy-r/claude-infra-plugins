#!/usr/bin/env bash
# Shared test helpers for customer-guard integration tests.
...
if cd "${_HELPERS_DIR}/../scripts" 2>/dev/null; then
    YCI_HOOK_SCRIPTS_DIR="$(pwd -P)"
    cd "${_HELPERS_DIR}" || true
else
    YCI_HOOK_SCRIPTS_DIR="${_HELPERS_DIR}/../scripts"
fi
export YCI_HOOK_SCRIPTS_DIR

# Shared customer-isolation scripts (extract-paths.py, etc.)
if cd "${_HELPERS_DIR}/../../../skills/_shared/customer-isolation/scripts" 2>/dev/null; then
    YCI_SCRIPTS_DIR="$(pwd -P)"
    cd "${_HELPERS_DIR}" || true
else
    YCI_SCRIPTS_DIR="${_HELPERS_DIR}/../../../skills/_shared/customer-isolation/scripts"
fi
export YCI_SCRIPTS_DIR

# Shared _shared scripts dir (resolve-data-root.sh)
if cd "${_HELPERS_DIR}/../../../skills/_shared/scripts" 2>/dev/null; then
    YCI_SHARED_DIR="$(pwd -P)"
    cd "${_HELPERS_DIR}" || true
else
    YCI_SHARED_DIR="${_HELPERS_DIR}/../../../skills/_shared/scripts"
fi
export YCI_SHARED_DIR

# References dir — defaults to customer-profile's references, then overridden below.
if cd "${_HELPERS_DIR}/../../../skills/customer-profile/references" 2>/dev/null; then
    YCI_REFS_DIR="$(pwd -P)"
    cd "${_HELPERS_DIR}" || true
else
    YCI_REFS_DIR="${_HELPERS_DIR}/../../../skills/customer-profile/references"
fi
export YCI_REFS_DIR

# Override: the customer-guard hook's error-messages.md lives in references/
# alongside this tests dir's parent (tests/ -> customer-guard/ -> references/).
...
# Repo root — needed to invoke pretool.sh with a stable path
# tests/ is 4 levels below repo root: yci/hooks/customer-guard/tests/
...
# ---------------------------------------------------------------------------
# Profile builder — writes a full-schema customer profile to <out>.
# Usage: _build_profile <out-path> <customer-id> <display-name> <inventory-rel-path>
# ---------------------------------------------------------------------------

_build_profile() {
    local out="$1" id="$2" name="$3" inv="$4"
    cat > "$out" <<EOF
customer:
  id: ${id}
  display_name: "${name}"
